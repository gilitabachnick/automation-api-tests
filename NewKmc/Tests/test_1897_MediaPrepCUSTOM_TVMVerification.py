...
@test_name: test_1897_MediaPrepCUSTOM_TVMVerification.py
 
 @desc : The test performs mediPrep custom distribution and then go to TVM app and perform files verification 
...
class TestClass:
    ...
    def setup_class(self):
        ...
        try:
            ...
            self.DistributionProfileSelectionText    = inifile.RetIniVal(section, 'DistributionProfileSelectionMediaPrepCustom')
            self.DistributionEntryName  = 'Wildlife'
            self.DistributionfilePth    = r'\Wildlife.wmv' 
            self.practitest = Practitest.practitest('4586')
            self.logi = reporter2.Reporter2('test_1897_MediaPrepCUSTOM_TVMVerification')
    ...
    def test_1897_MediaPrepCUSTOM_TVMVerification(self):
             
        global testStatus
        self.logi.initMsg('test_1897_MediaPrepCUSTOM_TVMVerification')
        ...
        try:            
            ...
            #Drilldown to entry by name.   
             ...
             self.DistributionEntryName = 'Wildlife_automation_mediaPrepCUSTOM_' + str(random.randint(1,101)) + "_" + x.strftime("%x")
             self.logi.appendMsg("INFO - Going to update entry name - " + self.DistributionEntryName)
             rc = self.entryPage.EntrySetBasicMetadata(self.DistributionEntryName,None,None,None,self.DistributionEntryName)#(self.DistributionEntryName)
             if rc == False:
             ...
             rc = self.BasicFuncs.verifyElement(self.Wd, self.logi,DOM.DISTRIBUTION_TAB, "Distribution tab", 15)
             if rc == False:
             ...
             if rc:
                ...
                #Login to TVM app
                 ...
                 # Verify TVM Files Number - Expected 2 files on custom mediaPrep case
                 rc = self.distribusionsFuncs.verifyTVMFilesNumber(self.Wd, self.logi, self.DistributionEntryName, 2)
                 if(rc):
    ...
    def teardown(self):
        ...
        if testStatus == False:
            ...
            self.practitest.post(Practi_TestSet_ID, '1897','1')
            assert False
        else:
            self.logi.reportTest('pass',"moran.cohen@kaltura.com")
            self.practitest.post(Practi_TestSet_ID, '1897','0')
            assert True         
            
            
    #===========================================================================
    # pytest.main('test_1897_MediaPrepCUSTOM_TVMVerification.py -s')
    #===========================================================================
