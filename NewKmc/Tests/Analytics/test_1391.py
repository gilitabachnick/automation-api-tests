...
class TestClass:
    ...
    def setup_class(self):
        ...
        try:
            ...
            pass
    #===========================================================================
    def test_1391_Analytics(self):

        pth = os.path.abspath(os.path.join(os.path.dirname(__file__), '..','..', 'UploadData'))
        csvObj = MyCsv.MyCsv(os.path.join(pth, 'csv_analytics_highlights.csv'))
            ...
            try:
                if csvObj.readValFromCsv(testNum, 0) == "yes":  # this test should run
                    ...
                    if testStatus == True:
                        self.general.waitForSpinnerToFinish()
                        if comparePeriod == 2:
                            if self.analyticsFuncs.setGraphPeriod("Daily") == False:
                                testStatus = False
                        elif self.analyticsFuncs.setGraphPeriod("Monthly") == False:
                            testStatus = False
                        self.general.waitForSpinnerToFinish()

                    # Collect highlights data and graph data and compare them for each metric
                    if testStatus == True:
                        if self.analyticsFuncs.verifyHighlights(isCompareMode, int(preset), comparePeriod) == False:
                            testStatus = False
                ...
                self.logi.appendMsg("TEST FAILED - Analytics highlights didn't passed")
                testStatus = False
    #===========================================================================
