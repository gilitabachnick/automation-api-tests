...
class TestClass:
    ...
    def setup_class(self):
        ...
        try:
            ...
            self.logi = reporter2.Reporter2('TEST1237 Reject Moderation')
    ...
    def test_1237(self):
        ...
        if testStatus == True:
            ...
            self.logi.initMsg('test 1237 Moderation > Reject Moderation')
            ...
            try:
                ...
                for i in range(0,2):
                    ...
                    if i==0:
                        ...
                        time.sleep(5)
                        self.logi.appendMsg("INFO - going to select 'Reject' moderation via Action Menu for single entry")
                        self.BasicFuncs.tblSelectAction(self.Wd, 0, "Reject")
                        
                    else:
                       self.BasicFuncs.searchEntrySimpleSearch(self.Wd, "test 1000_*")
                       time.sleep(5)
                       self.logi.appendMsg("INFO - going to select 'Reject' moderation via Bulk Action for multiple entries")
                       self.BasicFuncs.CheckUncheckRowInTable(self.Wd, 0)
                       self.BasicFuncs.bulkSelectAction(self.Wd, "Reject")
                       
                    time.sleep(5)
                    self.logi.appendMsg("INFO - going to check notification message for Reject media")
                    popupMessageText = self.Wd.find_element_by_xpath(DOM.MODERATION_REJECT_MSG)
                    msgtxt = popupMessageText.text
                    
                    if msgtxt.find("Moderation: Reject Media")>=0:
                    ...
                    self.logi.appendMsg("INFO - going to reject media by pressing on 'YES' button")
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
            ...
            self.practitest.post(Practi_TestSet_ID, '1237','1')
            assert False
        else:
            self.logi.reportTest('pass',self.sendto)
            self.practitest.post(Practi_TestSet_ID, '1237','0')
            assert True         
    ...
    #===========================================================================
    # pytest.main('test_1237_Moderation_Reject.py -s')
    #===========================================================================
