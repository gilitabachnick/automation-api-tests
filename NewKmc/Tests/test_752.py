...
class TestClass:
    ...
    def setup_class(self):
        ...
        try:
            ...
            self.logi = reporter2.Reporter2('TEST752')
    ...
    def test_752(self):
        ...
        try:  
            ...
            self.logi.appendMsg("INFO- going to verify that no entries created after closing the window")
            for fname in (dictFilename):
                entryStatus,lineText = self.BasicFuncs.waitForEntryStatusReady(self.Wd,dictFilename[fname])
                if lineText == "NoEntry":
                    self.logi.appendMsg("PASS - entry with the name " + dictFilename[fname] + " was not created as expected")
                else:
                    self.logi.appendMsg("FAIL - the entry " + dictFilename[fname] + " created while it should not be created")
                    testStatus = False
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
            self.logi.reportTest('fail',self.sendto)
            self.practitest.post(Practi_TestSet_ID, '752','1')
            assert False
        else:
            self.logi.reportTest('pass',self.sendto)
            self.practitest.post(Practi_TestSet_ID, '752','0')
            assert True         

    #===========================================================================
    if Run_locally:
        pytest.main(args=['test_752.py', '-s'])
    #===========================================================================
