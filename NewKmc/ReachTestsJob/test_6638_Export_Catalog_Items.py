# ==================================================================================================================
#  @Author: Zeev Shulman 12/9/21
#  @description :
#
#  Replaces the following tests in Reach manual regression/post production:
#  6638 - Export Reach Catalog Items to CSV from Admin Console
#
# ===================================================================================================================
...
class TestClass:
    ...
    def setup_class(self):
        try:
            ...
            self.logi = reporter2.Reporter2('test_6638_Export_Catalog_Items')
    ...
    def test_6638_Export_Catalog_Items(self):

        global testStatus
        # Invoke login - admin console
        try:

            self.logi.initMsg('test_6638_Export_Catalog_Items')
        ...
        try:
            self.logi.appendMsg("INFO - Going to navigate to Reach>Catalog Items")
            self.Wd.find_element_by_xpath(DOM.MACONSOLE_REACH_TAB).click()
            self.logi.appendMsg("PASS - navigated to Reach>Catalog Items")
        except Exception as e:
            print(e)
            testStatus = False
            self.logi.appendMsg("FAIL - FAILED to navigate to Catalog Items")
            return
        ...
        # ===============================================================
        try:
            self.logi.appendMsg("INFO - Going to filter the Catalog Items data")
            # Using the top 'Vendor Partner ID' - tr[1]/td[5] - to filter the table
            vendor_partner_id = self.Wd.find_element_by_xpath(RDOM.REACH_CATALOG_ITEMS_1_FIELDS.replace("TEXTTOREPLACE", "5")).text
            self.Wd.find_element_by_xpath(RDOM.REACH_FILTER_BY_PID).send_keys(vendor_partner_id)
            self.Wd.find_element_by_xpath(DOM.ADMINCONSOLE_SEARCH_BTN).click()
            time.sleep(1)

            # ===========================================================================
            # create validation data from the Catalog Items table
            # validation_data_str = the first entry's columns converted into a string
            # ===========================================================================
            validation_data_str = ""
            for i in range(1, 12):
                xp = RDOM.REACH_CATALOG_ITEMS_1_FIELDS.replace("TEXTTOREPLACE", str(i))
                box = self.Wd.find_element_by_xpath(xp).text
                validation_data_str = validation_data_str + box + ","
            self.logi.appendMsg("PASS - Catalog Items data found")
        except Exception as Exp:
            print(Exp)
            self.logi.appendMsg("FAIL - couldn't find the Catalog Items data")
            testStatus = False
            return
        ...
        try:
            self.logi.appendMsg("INFO - Going open Exported CSV")
    ...
    def teardown_class(self):
        ...
        try:
            if testStatus == False:
                self.practitest.post(Practi_TestSet_ID, '6638', '1')
                self.logi.reportTest('fail', self.sendto)
                assert False
            else:
                self.practitest.post(Practi_TestSet_ID, '6638', '0')
    ...
    # ===========================================================================
    if Run_locally:
        pytest.main(args=['test_6638_Export_Catalog_Items.py', '-s'])
    # ===========================================================================
