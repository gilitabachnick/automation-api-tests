...
@test_name: test_835_LiveNG_PreviewModeRecordingAPPEND_Trans_Sanity.py
 
 @desc : This test check E2E test of new LiveNG entries transcoding + PREVIEWMODE ON + Recording APPEND by Creating new live entry with explicitLive=1 enable  and start streaming and then
 ...
 Verify that the recorded entry is played OK from aws(because of this bug it's playing from klive).----> OPEN BUG https://kaltura.atlassian.net/browse/LIV-856
 Verify duration of the recording entry.
 STOP STREAMING
 Verify that the recorded entry is played OK from aws
 Verify that duration didn't change between END_LIVE to stop streaming state
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_835_LiveNG_PreviewModeRecordingAPPEND_Trans_Sanity')
        self.logi.initMsg('test_835_LiveNG_PreviewModeRecordingAPPEND_Trans_Sanity')
    ...
    def test_835_LiveNG_PreviewModeRecordingAPPEND_Trans_Sanity(self):
        global testStatus
        try: 
           ...
           # Create Admin ks
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-LiveEntry_PEVIEWMODE_RECORDING_APPEND_DVR_trans' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                # Waiting about 1 minutes after end live and then start to check the mp4 flavors upload status
                ...
                #After fixing LIV-856-->Need to change on code sniffer_fitler_After_Mp4flavorsUpload
                time.sleep(5)

                self.logi.appendMsg("INFO - AFTER END_LIVE(still Streaming):Going to play RECORDED ENTRY from " + self.sniffer_fitler_Before_Mp4flavorsUpload + " before(because LIV-856 klive) MP4 flavors uploaded " + str(entryLiveStream.recordedEntryId) + "  - MinToPlay=" + str(self.MinToPlay) + " , Start time = " + str(datetime.datetime.now()))
                limitTimeout = datetime.datetime.now() + datetime.timedelta(0, self.MinToPlay * 60)
                seenAll = False
                timeout = 0  # ************ ADD timeout for refresh player issue - No caption icon
                while datetime.datetime.now() <= limitTimeout and seenAll == False or timeout < 3:
                    time.sleep(5)
                    rc = self.liveObj.verifyAllEntriesPlayOrNoBbyQrcode(self.recorded_entrieslst, True,PlayerVersion=self.PlayerVersion,sniffer_fitler=self.sniffer_fitler_Before_Mp4flavorsUpload,Protocol="http",ServerURL=self.ServerURL)
                    time.sleep(5)
                    timeout = timeout + 1
                    if rc == False and timeout < 2:
                        self.logi.appendMsg("INFO - ******AFTER END_LIVE(still Streaming)::Going to play AGAIN after player CACHE ISSUE (No caption ICONF on the PLAYER)RECORDED ENTRY from " + self.sniffer_fitler_Before_Mp4flavorsUpload + " after MP4 flavors uploaded " + str(entryLiveStream.recordedEntryId) + "  - Try timeout_Cnt=" + str(timeout) + " , Start time = " + str(datetime.datetime.now()))
                        time.sleep(90)  # Time for player cache issue
                    if not rc and timeout >= 3:  # Change condition
                        print("FAIL -  AFTER END_LIVE(still Streaming):Going to play RECORDED ENTRY  after mp4 conversion - Retry count:timeout = " + str( timeout))
                        testStatus = False
                        return
                    if self.seenAll_justOnce_flag == True and rc != False:  # Change condition
                        seenAll = True
                        break
                self.logi.appendMsg("PASS - AFTER END_LIVE(still Streaming):RECORDED ENTRY Playback from " + self.sniffer_fitler_Before_Mp4flavorsUpload + " of " + str(entryLiveStream.recordedEntryId) + " after MP4 flavors uploaded - MinToPlay=" + str(self.MinToPlay) + " , End time = " + str(datetime.datetime.now()))
                ...
                #kill ffmpeg ps
                self.logi.appendMsg("INFO - Going to end streams by kill cmd FoundByProcessId .Datetime = " + str(datetime.datetime.now()))
                ...
                while (datetime.datetime.now() <= limitTimeout and seenAll == False) or timeout < 3:  # Change condition
                    ...
                    if rc == False and timeout < 2:
                        ...
                        time.sleep(90) #Time for player cache issue
                    if not rc and timeout >= 3:  # Change condition
                        print("FAIL - AFTER STOP/KILL STREAMING:Going to play RECORDED ENTRY after mp4 conversion - Retry count:timeout = " + str(timeout))
                        testStatus = False
                        return
                    if self.seenAll_justOnce_flag == True and rc != False:  # Change condition
                        seenAll = True
                ...
                ########### DURATION Compare for recording entry ---- >After stop streaming need to make sure that duration didn't change between END_LIVE to stop streaming state
                durationFirstRecording_AfterSTOPSTREAMING = int(self.client.baseEntry.get(FirstRecording_EntryID).duration)
                # Cal recording duration after mp4 flavors are uploaded
                self.logi.appendMsg("INFO - AFTER STOP/KILL STREAMING:Going to verify DURATION of Recording entry - Verify that duration didn't change between END_LIVE to stop streaming state.FirstRecording_EntryID = " + str(FirstRecording_EntryID))
                #recordingTime1 = int(END_LIVE_time - GO_LIVE_time)
                if durationFirstRecording_AfterSTOPSTREAMING == int(durationFirstRecording):
                    self.logi.appendMsg("PASS - AFTER STOP/KILL STREAMING:Recording entry:duration didn't change between END_LIVE to stop streaming state.FirstRecording - durationFirstRecording=" + str(durationFirstRecording) + " , durationFirstRecording_AfterSTOPSTREAMING = " + str(durationFirstRecording_AfterSTOPSTREAMING) + ", FirstRecording_EntryID = " + str(FirstRecording_EntryID))
                else:
                    self.logi.appendMsg("FAIL - AFTER STOP/KILL STREAMING:Recording entry:duration change between END_LIVE to stop streaming state.FirstRecording -  durationFirstRecording=" + str(durationFirstRecording)) + " , durationFirstRecording_AfterSTOPSTREAMING = " + str(durationFirstRecording_AfterSTOPSTREAMING) + ", FirstRecording_EntryID = " + str(FirstRecording_EntryID)
                    testStatus = False
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID,'835','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID,'835','0', self.logi.msg)
    ...
    #===========================================================================
    #pytest.main(['test_835_LiveNG_PreviewModeRecordingAPPEND_Trans_Sanity.py', '-s'])
    #===========================================================================
