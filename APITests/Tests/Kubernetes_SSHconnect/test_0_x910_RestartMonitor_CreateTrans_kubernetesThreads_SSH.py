...
class TestClass:
    ...
    def setup_class(self):
        ...
        #self.Kubernetes_Monitor = "--selector=component=monitor"
        self.Kubernetes_Component = "--selector=component=monitor" # cmd line for delete pod
        self.Kubernetes_SearchResult = f"""pod "monitor-"""  # Text for search after delete Component
        self.NumOfEntries = 1
    ...
    def test_0_910_RestartMonitor_CreateTrans_kubernetesThreads(self):
        ...
        try:
            ...
            for i in range(0, self.NumOfEntries):
                ...
                for currentRestart in range(1, self.NumOfRestartComponent):
                    self.logi.appendMsg("INFO - ####### Going to RESTART Monitor-" + str(currentRestart) + " :currentMonitor=" + str(self.Kubernetes_Component))
                    #self.KubernetesCmdLine_clusterType = "aws eks update-kubeconfig --name nvq1-eks-live-manager-1 --region us-east-1"
                    rc = self.KubernetesObj.Kubernetes_RestartComponent(host=self.Kubernetes_remote_host,user=self.Kubernetes_remote_user,passwd=self.Kubernetes_remote_pass,entryId=self.entryId,partnerId=self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl,currentComponent=self.Kubernetes_Component,KubernetesCmdLine_clusterType=self.KubernetesCmdLine_clusterType,SearchResult=self.Kubernetes_SearchResult)
                    if rc == True:
                        self.logi.appendMsg("PASS - Start_KubernetesByCmd - RESTART Monitor-" + str(currentRestart) + " is done.currentMonitor = " + str(self.Kubernetes_Component))
                    else:
                        self.logi.appendMsg("FAIL - Start_KubernetesByCmd-  RESTART Monitor-" + str(currentRestart) + " is failed. currentMonitor = " + str(self.Kubernetes_Component))
                        testStatus = False
                        return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(Practi_TestSet_ID, '910','1')
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(Practi_TestSet_ID, '910','0')
