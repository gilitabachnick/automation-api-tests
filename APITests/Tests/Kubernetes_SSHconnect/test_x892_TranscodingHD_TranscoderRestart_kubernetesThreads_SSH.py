...
class TestClass:
    ...
    def setup_class(self):
        ...
        if isProd:
            ...
            self.Live_Change_Cluster = ast.literal_eval(
                inifile.RetIniVal(section, 'Live_Change_Cluster'))  # import ast # Change string(False/True) to BOOL
    ...
    def test_892_TranscodingHD_TranscoderRestart_kubernetesThreads(self):
        ...
        try:
            ...
            for i in range(0, self.NumOfEntries):
                ...
                self.logi.appendMsg("INFO - Going to get transcoder by entryId = " + str(self.entryId))
                currentTranscoder=self.KubernetesObj.Kubernetes_getTranscoderByEntryId(self.Kubernetes_remote_host,self.Kubernetes_remote_user,self.Kubernetes_remote_pass,KubernetesCmdLine_clusterType=self.KubernetesCmdLine_clusterType,entryId=self.entryId,partnerId=self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl)
                ...
                self.logi.appendMsg("INFO - ********* Going to RESTART Transcoder = " + str(currentTranscoder))
                rc = self.KubernetesObj.Kubernetes_RestartComponent(self.Kubernetes_remote_host,self.Kubernetes_remote_user,self.Kubernetes_remote_pass,entryId=self.entryId,partnerId=self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl,currentComponent=currentTranscoder)
                if rc == True:
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(Practi_TestSet_ID, '892','1')
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(Practi_TestSet_ID, '892','0')
