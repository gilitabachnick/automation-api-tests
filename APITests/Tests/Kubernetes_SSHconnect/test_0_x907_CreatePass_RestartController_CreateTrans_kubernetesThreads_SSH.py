...
class TestClass:
    ...
    def setup_class(self):
        ...
        #self.Kubernetes_Controller = "--selector=component=controller"
        self.Kubernetes_Component = "--selector=component=controller"  # cmd line for delete pod
        self.Kubernetes_SearchResult = f"""pod "controllers-"""  # Text for search after delete Component
        self.NumOfEntries = 1
    ...
    def test_0_907_CreatePass_RestartController_CreateTrans_kubernetesThreads(self):
        ...
        try:
            ...
            for i in range(0, self.NumOfEntries):
                ...
                self.logi.appendMsg("INFO - ####### Going to RESTART Controller:currentController=" + str(self.Kubernetes_Component))
                #self.KubernetesCmdLine_clusterType = "aws eks update-kubeconfig --name nvq1-eks-live-manager-1 --region us-east-1"
                ...
                if rc == True:
                    self.logi.appendMsg("PASS - Start_KubernetesByCmd - RESTART Controller is done.currentController = " + str(self.Kubernetes_Component))
                else:
                    self.logi.appendMsg("FAIL - Start_KubernetesByCmd-  RESTART Controller is failed. currentController = " + str(self.Kubernetes_Component))
                    testStatus = False
                    return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(Practi_TestSet_ID, '907','1')
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(Practi_TestSet_ID, '907','0')
