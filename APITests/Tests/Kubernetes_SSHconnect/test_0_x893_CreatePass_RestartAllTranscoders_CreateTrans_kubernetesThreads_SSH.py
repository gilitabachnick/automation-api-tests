...
@test_name: test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads.py
 ...
 Check kubernetes only on QA env:
     GET all transcorders of QA env
     kubectl delete pod for all transcoders
     Verify LIVE entry playback passthrough by QRCODE during threading
 ...
 'Next tasks --> .ts;.m3u8' --> respone different from 200->print request url ---> Next working on
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 '''
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads')
        self.logi.initMsg('test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads')
    ...
    def test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads(self):
        global testStatus
        try:
            if self.env =='prod':
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-AllTranscodersRestart_Passthrough_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if self.Live_Change_Cluster == True and self.env == 'testing':
                    ...
                    if rc == True:
                        ...
                        testStatus = False
                        return
                # get All transcoders By EntryId
                self.logi.appendMsg("INFO - Going to get All transcoders")
                arrTranscoders=self.KubernetesObj.Kubernetes_getAllTranscoders(self.Kubernetes_remote_host,self.Kubernetes_remote_user,self.Kubernetes_remote_pass,KubernetesCmdLine_clusterType=self.KubernetesCmdLine_clusterType,entryId=self.entryId,partnerId=self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl)
                if arrTranscoders==False:
                    self.logi.appendMsg("FAIL - Kubernetes_getTranscoderByEntryId .Get packager by entryID = " + self.entryId)
                    testStatus = False
                    return
                ...
                if self.Live_Change_Cluster == True:
                    ...
                    self.KubernetesCmdLine_clusterType = "aws eks update-kubeconfig --name nvq1-eks-live-cluster-1-a --region us-east-1"
                self.logi.appendMsg("INFO - ********* Going to RESTART All Transcoders:")
                for j in range(0, len(arrTranscoders)):
                    currentTranscoder=arrTranscoders[j][0]
                    self.logi.appendMsg("INFO - ####### Going to RESTART Transcoder-" + str(j) + " :currentTranscoder=" + str(currentTranscoder))
                    rc = self.KubernetesObj.Kubernetes_RestartComponent(host=self.Kubernetes_remote_host,user=self.Kubernetes_remote_user,passwd=self.Kubernetes_remote_pass,entryId=self.entryId,partnerId=self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl,currentComponent=currentTranscoder)
                    if rc == True:
                        self.logi.appendMsg("PASS - Start_KubernetesByCmd - RESTART Transcoder is done.currentTranscoder = " + str(currentTranscoder))
                    else:
                        self.logi.appendMsg("FAIL - Start_KubernetesByCmd-  RESTART Transcoder is failed. currentTranscoder = " + str(currentTranscoder))
                        testStatus = False
                        return
            ...
            # Create self.NumOfEntries live entries
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client,'AUTOMATION-AllTranscodersRestart_TranscodingHD_' + str(i) + '_' + str(datetime.datetime.now()),"Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag","Admintag", "Moran category", 1, None, self.CloudtranscodeId)
                self.entry = Entryobj.AddEntryLiveStream(None, None, 0, 0)
                self.entrieslst.append(self.entry)
                self.testTeardownclass.addTearCommand(Entryobj, 'DeleteEntry(\'' + str(self.entry.id) + '\')')
                streamUrl = self.client.liveStream.get(self.entry.id)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if self.Live_Change_Cluster == True and self.env == 'testing':
                    Current_primaryBroadcastingUrl = Current_primaryBroadcastingUrl.replace("1-a",self.Live_Cluster_Primary)
                self.logi.appendMsg("INFO - ************** Going to RESTART ssh Start_StreamEntryByffmpegCmd after restart all transcoders.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , " + self.url + " , " + Current_primaryBroadcastingUrl)
                ffmpegCmdLine = self.liveObj.ffmpegCmdString(self.filePath,str(Current_primaryBroadcastingUrl),self.entryId)
                start_streaming = datetime.datetime.now().timestamp()
                rc, ffmpegOutputString = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host,self.remote_user,self.remote_pass, ffmpegCmdLine,self.entryId, self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl,FoundByProcessId=True)
                if rc == False:
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(Practi_TestSet_ID, '893','1')
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(Practi_TestSet_ID, '893','0')
           self.logi.reportTest('pass',self.sendto)
           assert True


    #===========================================================================
    #pytest.main('test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads.py -s')
    #===========================================================================
