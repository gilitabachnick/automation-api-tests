...
class TestClass:
    ...
    def setup_class(self):
        ...
        # create client session for NY
        self.logi.appendMsg('start create session for partenr: ' + self.PublisherID)
        mySess = ClienSession.clientSession(self.PublisherID,self.NySrverUrl,self.UserSecret)
        self.clientNY = mySess.OpenSession()
        time.sleep(2)
        ...
        for i in range(0,36):
            if i<=17:
                ...
                Entryobj = Entry.Entry(self.clientNY, self.primaryBroadcastingUrllst[i-18] + '_' + str(datetime.datetime.now()), "Live Automation test", "Live tag", "Admintag", "adi category", 1,None,self.sourceonlyId)
    ...
    def test_LiveProdServers(self):
        ...
        for i in range(0,22):
            ...
            for j in range(0,2):
                if j==0:
                    self.driver = liveObj.Entryandstrem(self.sourceonlyId, 0, 0, self.entrieslst[i], self.primaryBroadcastingUrllst[i],'/home/ubuntu/MediaUpload/QR_05_minutes.mp4')   
                    ...
                    self.driver = liveObj.Entryandstrem(self.sourceonlyId, 0, 0, self.entrieslst[i+18], self.primaryBroadcastingUrllst[i],'/home/ubuntu/MediaUpload/QR_05_minutes.mp4')
                    negmailMessage = 'FAIL - Stream from server ' + str(self.primaryBroadcastingUrllst[i]) + ' did not worked with entry Id=' + str(self.entrieslst[i+18].id)
                    posmessage = 'PASS - Stream from server ' + str(self.primaryBroadcastingUrllst[i]) + ' worked OK with entry Id=' + str(self.entrieslst[i+18].id)
...
#===============================================================================
#pytest.main('test_LiveProdServers.py -s')        
#===============================================================================
