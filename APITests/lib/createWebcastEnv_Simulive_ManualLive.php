...
$partnerID = $config[$env]['partnerID'] ;
$adminSecret = $config[$env]['adminSecret'];
$serviceUrl = $config[$env]['serviceUrl'];
$conversionProfileID = $config[$env]['transcodingProfile'];
$kwebcastProfileId = $config[$env]['entryMetadataProfileId'];
$eventsProfileId = $config[$env]['eventsProfileId'];
//$isSimulive = $config[$env]['isSimulive'];
$isSimulive = $argv[4];
print($isSimulive);
$vodId = $config[$env]['vodId'];
