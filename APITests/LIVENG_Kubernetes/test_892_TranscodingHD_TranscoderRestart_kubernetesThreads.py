...
@test_name: test_892_TranscodingHD_TranscoderRestart_kubernetesThreads.py
 ...
 Check kubernetes only on QA env:
     GET transcorder of entryId
     kubectl delete pod by current transcoder
...
2)kubectl exec -it analyzers-0 -- sh -c "python3 redis-cli.py --keys transcoder:\*0_wa6ugqel\*"
3)kubectl delete pod transcoding-agent-569c6bc8c8-7vzc5

Cmd line:
[live@ip-10-200-145-39 ~]$ kubectl exec -it analyzers-0 -- sh -c "python3 redis-cli.py --keys transcoder:\*0_wa6ugqel\*"
transcoder:0_wa6ugqel:4db2ac4cda21@cluster-1-a.live.nvq1

kubectl delete pod transcoding-agent-569c6bc8c8-7vzc5
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 '''
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_892_TranscodingHD_TranscoderRestart_kubernetesThreads')
        self.logi.initMsg('test_892_TranscodingHD_TranscoderRestart_kubernetesThreads')
    ...
    def test_892_TranscodingHD_TranscoderRestart_kubernetesThreads(self):
        global testStatus
        try:
            if self.env =='prod':
            ...
            ''' RETRIEVE TRANSCODING ID AND Transcoding HD IF NOT EXIST'''
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-TranscoderRestart_TranscodingHD_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                ###########################################GET Transcoder by entryID
                time.sleep(5)  # added
                if self.ConfigId != "":  # Do if multi regional setting
                    self.ConfigId = self.ConfigId + "-"
                # get Packager By EntryId
                self.logi.appendMsg("INFO - Going to get Transcoder by entryId = " + str(self.entryId) + ", datetime = " + str(datetime.datetime.now()))
                currentTranscoder = self.KubernetesObj.getTranscoderForEntry(entry_id= self.ConfigId + self.entryId, session_type='0',cluster_id=self.Live_Cluster_Primary)
                print(currentTranscoder)
                if currentTranscoder == False:
                    self.logi.appendMsg("FAIL - kubernetes - getTranscoderForEntry.entryId=" + str(self.entryId) + ", datetime = " + str(datetime.datetime.now()))
                    testStatus = False
                    return
                else:
                    self.logi.appendMsg("PASS - kubernetes - getTranscoderForEntry.entryId=" + str(self.entryId) + ", currentTranscoder " + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
                ...
                ############################# RESTART Transcoder ################################
                time.sleep(20)  # was 5
                self.logi.appendMsg("INFO - ********* Going to RESTART Transcoder = " + str(currentTranscoder[0]) + ", datetime = " + str(datetime.datetime.now()))
                rc = self.KubernetesObj.deletePod(cluster_id=self.Live_Cluster_Primary, pod_name=currentTranscoder[0])
                print(rc)
                if rc == False:
                    self.logi.appendMsg("FAIL - kubernetes - k8sConnect_deletePod - RESTART Transcoder is failed. currentTranscoder = " + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
                    testStatus = False
                    return
                else:
                    self.logi.appendMsg("PASS - kubernetes - k8sConnect_deletePod - RESTART Transcoder is done.currentTranscoder = " + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '892','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '892','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True


    #===========================================================================
    #pytest.main('test_892_TranscodingHD_TranscoderRestart_kubernetesThreads.py -s')
    #===========================================================================
