...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.transcoder_selector = "component=transcoding-agent"
    ...
    def test_0_893_CreatePass_RestartAllTranscoders_CreateTrans_kubernetesThreads(self):
        ...
        try:
            ...
            for i in range(0, self.NumOfEntries):
                ...
                arrTranscoders = self.KubernetesObj.listPod(self.Live_Cluster_Primary, self.transcoder_selector)
                if arrTranscoders == False:
                    self.logi.appendMsg("FAIL - Kubernetes - listPod .Get listPod by transcoder_selector = " + self.transcoder_selector)
                    testStatus = False
                    return
                self.logi.appendMsg("INFO - Kubernetes - Return transcoder listPod:")
                for j in range(0, len(arrTranscoders)):
                    currentTranscoder = arrTranscoders[j]
                    self.logi.appendMsg("INFO - Kubernetes - listPod .currentTranscoder = " + currentTranscoder)
                ...
                time.sleep(5)
                self.logi.appendMsg("INFO - ********* Going to RESTART All Transcoders:" + ", datetime = " + str(datetime.datetime.now()))
                for j in range(0, len(arrTranscoders)):
                    currentTranscoder = arrTranscoders[j]#[0]
                    self.logi.appendMsg("INFO - ####### Going to RESTART Transcoder-" + str(j) + " :currentTranscoder=" + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
                    rc = self.KubernetesObj.deletePod(cluster_id=self.Live_Cluster_Primary,pod_name=currentTranscoder)
                    if rc == False:
                        self.logi.appendMsg("FAIL - Start_KubernetesByCmd-  RESTART Transcoder is failed. currentTranscoder = " + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
                        testStatus = False
                        return
                    else:
                        self.logi.appendMsg("PASS - Start_KubernetesByCmd - RESTART Transcoder is done.currentTranscoder = " + str(currentTranscoder) + ", datetime = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '893','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '893','0', self.logi.msg)
