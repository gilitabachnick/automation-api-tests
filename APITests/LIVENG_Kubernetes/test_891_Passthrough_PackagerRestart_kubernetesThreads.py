...
@test_name: test_891_Passthrough_PackagerRestart_kubernetesThreads.py
...
Result:
4)kgp
Verify that relevant packager is restarted

Cmd line:
[live@ip-10-200-145-39 ~]$ kubectl exec -it analyzers-0 -- sh -c "python3 redis-cli.py --get CHANNEL_ASSOCIATION:0_wa6ugqel_0" | awk '{print substr($1,15,12)}' | sed -e 's/^"//' -e 's/"$//'
packager-2
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.QrCodecheckProgress = 8# was 4
        #=======================================================================
        ...
        self.logi = reporter2.Reporter2('test_891_Passthrough_PackagerRestart_kubernetesThreads')
        self.logi.initMsg('test_891_Passthrough_PackagerRestart_kubernetesThreads')
    ...
    def test_891_Passthrough_PackagerRestart_kubernetesThreads(self):
        global testStatus
        try:
            if self.env =='prod':
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-PackagerRestart_PASS_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd from Primary only.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , " + Current_primaryBroadcastingUrl)
                if self.IsSRT == True:
                ...
                ############################# RESTART packager ################################
                time.sleep(20)  # was 5
                self.logi.appendMsg("INFO - ********* Going to RESTART packager = " + str(currentPackager) + ", datetime = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '891','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '891','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True


    #===========================================================================
    #pytest.main('test_891_Passthrough_PackagerRestart_kubernetesThreads.py -s')
    #===========================================================================
