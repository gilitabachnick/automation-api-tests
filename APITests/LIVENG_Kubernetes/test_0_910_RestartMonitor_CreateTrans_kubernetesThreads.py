...
@test_name: test_0_910_RestartMonitor_CreateTrans_kubernetesThreads.py
 ...
 Check kubernetes only on QA env:
     ...
     kubectl kubectl delete pod --selector=component=monitor
...
Restart controller --> kubectl delete pod --selector=component=monitor
    Verify that the playback of the live entry is ok.
    Verify that it should not influence any of running streams.
    Cause failure to who trying to connect new stream at the moment that the monitor is down
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 '''
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.monitor_selector = "component=monitor"
        ...
        self.NumOfRestartComponent = 3 # Number of time to do restart to component like monitor
        ...
        self.logi = reporter2.Reporter2('test_0_910_RestartMonitor_CreateTrans_kubernetesThreads')
        self.logi.initMsg('test_0_910_RestartMonitor_CreateTrans_kubernetesThreads')
    ...
    def test_0_910_RestartMonitor_CreateTrans_kubernetesThreads(self):
        global testStatus
        try:
            if self.env =='prod':
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-MonitorRestart_Transcoding_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                ###### Get All monitors
                time.sleep(5)  # added
                self.logi.appendMsg("INFO - Going to get All monitors (Just one on QA env)")
                arrMonitors = self.KubernetesObj.listPod(self.Live_Cluster_Primary, self.monitor_selector)
                if arrMonitors == False:
                    self.logi.appendMsg("FAIL - Kubernetes - listPod .Get listPod by monitor_selector = " + self.monitor_selector)
                    testStatus = False
                    return
                self.logi.appendMsg("INFO - Kubernetes - Return monitor listPod:")
                if len(arrMonitors) != 1:
                    self.logi.appendMsg("FAIL - Kubernetes - listPod . Return more than one monitor on listPod by monitor_selector = " + self.monitor_selector + ", datetime = " + str(datetime.datetime.now()))
                for j in range(0, len(arrMonitors)):
                    currentMonitor = arrMonitors[j]
                    self.logi.appendMsg("INFO - Kubernetes - listPod .currentTranscoder = " + str(currentMonitor))
                ...
                time.sleep(5)  # added
                self.logi.appendMsg("INFO - ********* Going to RESTART Monitor:" + ", datetime = " + str(datetime.datetime.now()))
                for j in range(0, len(arrMonitors)):
                    currentMonitor = arrMonitors[j]  # [0]
                    self.logi.appendMsg("INFO - ####### Going to RESTART Monitor-" + str(j) + " :currentMonitor=" + str(currentMonitor) + ", datetime = " + str(datetime.datetime.now()))
                    rc = self.KubernetesObj.deletePod(cluster_id=self.Live_Cluster_Primary, pod_name=currentMonitor)
                    if rc == False:
                        self.logi.appendMsg("FAIL - Start_KubernetesByCmd-  RESTART Monitor is failed. currentMonitor = " + str(currentMonitor) + ", datetime = " + str(datetime.datetime.now()))
                        testStatus = False
                        return

                    else:
                         self.logi.appendMsg("PASS - Start_KubernetesByCmd - RESTART Monitor is done.currentMonitor = " + str(currentMonitor) + ", datetime = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '910','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '910','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True


    #===========================================================================
    #pytest.main('test_0_910_RestartMonitor_CreateTrans_kubernetesThreads.py -s')
    #===========================================================================
