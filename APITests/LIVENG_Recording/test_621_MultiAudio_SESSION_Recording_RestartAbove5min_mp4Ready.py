...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
@author: moran.cohen
@test_name: test_621_MultiAudio_SESSION_Recording_RestartAbove5min_mp4Ready.py
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_621_MultiAudio_SESSION_Recording_RestartAbove5min_mp4Ready')
        self.logi.initMsg('test_621_MultiAudio_SESSION_Recording_RestartAbove5min_mp4Ready')
    ...
    def test_621_MultiAudio_SESSION_Recording_RestartAbove5min_mp4Ready(self):
        global testStatus
        try:
           ...
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                ...
                ffmpegCmdLine = self.liveObj.ffmpegCmdString_MultiAudio(self.filePath,str(Current_primaryBroadcastingUrl),self.streamUrl[i].streamName)
            ...
            while datetime.datetime.now() <= limitTimeout and seenAll==False:
                    rc = self.liveObj.verifyAllEntriesPlayOrNoBbyOnlyPlayback(self.entrieslst, True,PlayerVersion=self.PlayerVersion,MultiAudio=True,ServerURL=self.ServerURL)
                    time.sleep(5)
                    if not rc:
                        testStatus = False
                        return
            ...
            while (datetime.datetime.now() <= limitTimeout and seenAll == False) or timeout < 3:  # Change condition
                rc = self.liveObj.verifyAllEntriesPlayOrNoBbyOnlyPlayback(self.recorded_entrieslst, True,PlayerVersion=self.PlayerVersion,MultiAudio=True,sniffer_fitler=self.sniffer_fitler_Before_Mp4flavorsUpload,Protocol="http",ServerURL=self.ServerURL)
                time.sleep(5)
                timeout = timeout + 1
                if rc == False and timeout < 2:
                    self.logi.appendMsg("INFO - ****** Going to play AGAIN after player CACHE ISSUE RECORDED ENTRY from " + self.sniffer_fitler_Before_Mp4flavorsUpload + " before MP4 flavors uploaded " + str(entryLiveStream.recordedEntryId) + "  - Try timeout_Cnt=" + str(timeout) + " , Start time = " + str(datetime.datetime.now()))
                    time.sleep(90)  # Time for player cache issue
                if not rc and timeout >= 3:  # Change condition
                    print("FAIL - Going to play RECORDED ENTRY with multi audio before mp4 conversion - Retry count:timeout = " + str(timeout))
                    testStatus = False
                    return
            ...
            while datetime.datetime.now() <= limitTimeout and seenAll == False:
                if self.env == 'prod':
                    ...
                    languageList = "Spanish;English"
                rc = self.liveObj.verifyAllEntriesPlayOrNoBbyOnlyPlayback(self.recorded_entrieslst, True,PlayerVersion=self.PlayerVersion,MultiAudio=True,sniffer_fitler=self.sniffer_fitler_After_Mp4flavorsUpload,Protocol="http",languageList=languageList,ServerURL=self.ServerURL)
                time.sleep(5)
                if not rc:
                    testStatus = False
                    return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID,'621','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID,'621','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_621_MultiAudio_SESSION_Recording_RestartAbove5min_mp4Ready.py -s')
    #===========================================================================
