...
@test_name: test_804_SESSION_Recording_Pass_Sanity_redirectEntry.py
 @desc : this test check E2E test of new LiveNG entries passthrough + SESSION recording with RTMP streaming by new logic function - host access run ffmpeg cmd
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_804_SESSION_Recording_Pass_Sanity_redirectEntry')
        self.logi.initMsg('test_804_SESSION_Recording_Pass_Sanity_redirectEntry')
    ...
    def test_804_SESSION_Recording_Pass_Sanity_redirectEntry(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-RECORDING_SESSION_SANITY_Pass_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            rc = self.liveObj.WaitForFlavorsMP4uploaded(self.client,entryLiveStream.recordedEntryId, recorded_entry,expectedFlavors_totalCount=1) #expectedFlavors_totalCount=1 for passthrough
            if rc == True:
            ...
            self.logi.appendMsg("INFO - STOP STREAMING(after mp4 uploaded):Going to play RECORDED ENTRY from " + self.sniffer_fitler_After_Mp4flavorsUpload + " after MP4 flavors uploaded " + str(entryLiveStream.recordedEntryId) + "  - MinToPlay=" + str(self.MinToPlay) + " , Start time = " + str(datetime.datetime.now()))
            limitTimeout = datetime.datetime.now() + datetime.timedelta(0, self.MinToPlay * 60)
            seenAll = False
            while datetime.datetime.now() <= limitTimeout and seenAll == False:
            ...
            self.logi.appendMsg("PASS - STOP STREAMING(after mp4 uploaded):RECORDED ENTRY Playback from " + self.sniffer_fitler_After_Mp4flavorsUpload + " of " + str(entryLiveStream.recordedEntryId) + " after MP4 flavors uploaded - MinToPlay=" + str(self.MinToPlay) + " , End time = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '804','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '804','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_804_SESSION_Recording_Pass_Sanity_redirectEntry.py -s')
    #===========================================================================
