...
@test_name: test_693_SESSION_Recording_Pass_LIVEClipping_Sanity.py
 @desc : this test check E2E test of new LiveNG entries passthrough + SESSION recording with LIVEClipping logic function
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_693_SESSION_Recording_Pass_LIVEClipping_Sanity')
        self.logi.initMsg('test_693_SESSION_Recording_Pass_LIVEClipping_Sanity')
    ...
    def test_693_SESSION_Recording_Pass_LIVEClipping_Sanity(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-RECORDING_SESSION_LiveClipping_SANITY_Pass_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            while (datetime.datetime.now() <= limitTimeout and seenAll == False) or timeout < 3:  # Change condition
                ...
                if rc == False and timeout < 2:
                    self.logi.appendMsg("INFO - ****** CLIPPED_ENTRY:Going to play AGAIN - player CACHE ISSUE CLIPPED ENTRY from " + self.sniffer_fitler_Before_Mp4flavorsUpload + " after MP4 flavors uploaded " + str(ClipEntry_entry.id) + "  - Try timeout_Cnt=" + str(timeout) + " , Start time = " + str(datetime.datetime.now()))
                    time.sleep(90)  # Time for player cache issue
                if not rc and timeout >= 3:  # Change condition
                    print("FAIL - CLIPPED_ENTRY:TIMEOUT-Going to play CLIPPED_ENTRYY before mp4 conversion - Retry count:timeout = " + str(timeout))
                    testStatus = False
                    return
            ...
            rc = self.liveObj.WaitForFlavorsMP4uploaded(self.client,ClipEntry_entry.id, recorded_entry,expectedFlavors_totalCount=1) #expectedFlavors_totalCount=1 for passthrough
            if rc == True:
           ...
           # CLIPPED ENTRY after MP4 flavors are uploaded - Playback verification of all entries
            ...
            while (datetime.datetime.now() <= limitTimeout and seenAll == False) or timeout < 3:  # Change condition
                ...
                timeout = timeout + 1
                if rc == False and timeout < 2:
                    self.logi.appendMsg("INFO - ****** CLIPPED_ENTRY:Going to play AGAIN after player CACHE ISSUE CLIPPED ENTRY from " + self.sniffer_fitler_After_Mp4flavorsUpload + " after MP4 flavors uploaded " + str(ClipEntry_entry.id) + "  - Try timeout_Cnt=" + str(timeout) + " , Start time = " + str(datetime.datetime.now()))
                    time.sleep(90)  # Time for player cache issue
                if not rc and timeout >= 3:  # Change condition
                    print("FAIL - CLIPPED_ENTRY:TIMEOUT-Going to play CLIPPED_ENTRYY after mp4 conversion - Retry count:timeout = " + str(timeout))
                    testStatus = False
                    return
                if self.seenAll_justOnce_flag == True and rc != False:  # Change condition
                    seenAll = True
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '693','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '693','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_693_SESSION_Recording_Pass_LIVEClipping_Sanity.py -s')
    #===========================================================================
