...
class TestClass:
    ...
    def setup_class(self):
        try:
            ...
            self.logi = reporter2.Reporter2('test_419_Leaderboard_LIVE_view')
    ...
    def test_419_Leaderboard_LIVE_view(self):
        global testStatus
        self.logi.initMsg('test_419_Leaderboard_LIVE_view')
        try:
            print("-------------- View Normal --------------")
            print("Info - select random user, view 1 min of a LIVE entry, get 5 points")
            lbId = 5
            ...
            entryID = LeaderBoardFuncs.ret_rnd_live_entry()
            print("Before - " + userID_str + " rank:" + str(list_before.objects[n].rank) + ", score:" + str(
                list_before.objects[n].score))
            r = LeaderBoardFuncs.playEntry(self, userID_str, entryID, 60)
            # r = LeaderBoardFuncs.playEntry(self, userID_str, "1_7nt8xaf7", 2000) # for testing 150 max points
            ...
            print("Info - view 1 min of LIVE __optional, get NO points")
            optional_before = list_after
            # 1_031q4wt6 is in __optional category
            r = LeaderBoardFuncs.playEntry(self, userID_str, "1_031q4wt6", 60)
            if r == False:
                print("FAIL - playEntry")
                testStatus = False
            ...
            exhausted_usr = "LB_usr56"
            exhausted_entry = "1_7nt8xaf7"
            print("Info -  user " + exhausted_usr+", view 1 min of previewed EXHAUSTED, 150 points max, entry " + exhausted_entry + ", get NO points")
            ...
            print("-------------- EXCLUDED user --------------")
            print("Info - select random EXCLUDED user, view 1 min of LIVE entry, get NO points")
    ...
    def teardown_class(self):
        ...
        try:
            if testStatus == True:
                print("  *** PASS ***")
                self.practitest.post(Practi_TestSet_ID, '419', '0')
                assert True
            else:
                print("  *** FAIL ***")
                self.practitest.post(Practi_TestSet_ID, '419', '1')
                assert False
        except Exception as Exp:
            print(Exp)
    ...
    # ===========================================================================
    if Run_locally:
        pytest.main(args=['test_X419_Leaderboard_LIVE_view.py', '-s'])
    # ===========================================================================