...
@test_name: test_979_Load_LiveCaptionReachPass.py
 @desc : this test check E2E test of new LiveNG entries Passthrough with live caption - Can set NumOfEntries
Test case:
Create live entry
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.remote_host = self.inifile.RetIniVal('NewLive', 'remote_host_LiveNG_LiveCaption_4')
        self.remote_user = self.inifile.RetIniVal('NewLive', 'remote_user_LiveNG_LiveCaption_4')
        self.remote_pass = ""
        self.filePath = "/home/kaltura/entries/LongCloDvRec.mp4"      

        self.NumOfEntries = 10
        self.MinToPlay = 30 # Minutes to play all entries
        ...
        self.SchedulerEvent_sessionEndTimeOffset = 60  # Time to add for EndTime from startTime  - minutes
        ...
        self.logi = reporter2.Reporter2('test_0_979_Load_LiveCaptionReachPass')
        self.logi.initMsg('test_0_979_Load_LiveCaptionReachPass')
    ...
    def test_0_979_Load_LiveCaptionReachPass(self):
        global testStatus
        arrffmpegOutputString=[]
        try:
            ...
            self.entrieslst = []
            self.streamUrl = []
            arr_EntryVendorTaskId = []
            self.logi.appendMsg('INFO - Going to create number of live entries with live caption = ' + str(self.NumOfEntries))
            # Create self.NumOfEntries live entries
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-LiveEntry_LiveCaptionReach_Pass_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
                ...
                if rc != True:
                    ...
                    testStatus = False
                    return
                arr_EntryVendorTaskId.append(EntryVendorTaskId)
            ...
            while startTime > time.time():#Wait until SchedulerEvent.startTime
                time.sleep(5)
            for i in range(0, len(arr_EntryVendorTaskId)):
                ...
                rc = self.liveObj.Create_KalturaEntryVendorTask_VerifyStatus(client=self.client, id=arr_EntryVendorTaskId[i],Expected_STATUS=3)  # entryVendorTask.status - first 1 scheduled , 3 processing
                if rc == False:
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if rc == False:
                    ...
                    testStatus = False
                    return
                arrffmpegOutputString.append(ffmpegOutputString)
                time.sleep(1)
            ...
            #kill ffmpeg ps
            for i in range(0, len(arrffmpegOutputString)):
                self.logi.appendMsg("INFO - Going to end streams by killall cmd.Datetime = " + str(datetime.datetime.now()))
                rc = self.liveObj.End_StreamEntryByffmpegCmd(self.remote_host, self.remote_user, self.remote_pass, self.entrieslst[i].id,self.PublisherID,FoundByProcessId=arrffmpegOutputString[i])
                if rc != False:
                    self.logi.appendMsg("PASS - streamEntryByShellScript and VerifyResultFromStreamEntryByShellScript.  streamEntryByShellScript: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entrieslst[i].id + " , " + self.PublisherID + " , " + str(self.playerId))
                else:
                    self.logi.appendMsg("FAIL - streamEntryByShellScript and VerifyResultFromStreamEntryByShellScript. streamEntryByShellScript: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entrieslst[i].id + " , " + self.PublisherID + " , " + str(self.playerId))
                    testStatus = False
                    return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '979','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '979','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_979_Load_LiveCaptionReachPass.py -s')
    #===========================================================================
