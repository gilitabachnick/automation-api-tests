...
@test_name: test_778_RTMPS_LiveNG_BothPrimaryAndBackup_Passthough.py.
 
 @desc : This test check E2E test of new LiveNG entries RTMPS streaming both Primary +  Backup -> Same URL playback - Passthough.
 creating New Player v2 + v3 
 Start RTMPS streaming by new logic function - host access run ffmpeg cmd.
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_778_RTMPS_LiveNG_BothPrimaryAndBackup_Passthough')
        self.logi.initMsg('test_778_RTMPS_LiveNG_BothPrimaryAndBackup_Passthough')
    ...
    def test_778_RTMPS_LiveNG_BothPrimaryAndBackup_Passthough(self):
        global testStatus
        try: 
           # create client session
            ...
            # Stream only backup - > Get entryId and secondarySecuredBroadcastingUrl from live entries
            for i in range(0, self.NumOfEntries):
                self.entryId = str(self.entrieslst[i].id)
                Current_primarySecuredBroadcastingUrl = self.streamUrl[i].primarySecuredBroadcastingUrl
                Current_secondarySecuredBroadcastingUrl = self.streamUrl[i].secondarySecuredBroadcastingUrl
                if self.env == 'testing':
                    if self.Live_Change_Cluster == True:
                        Current_primarySecuredBroadcastingUrl = Current_primarySecuredBroadcastingUrl.replace("1-a",self.Live_Cluster_Primary)
                        Current_secondarySecuredBroadcastingUrl = Current_secondarySecuredBroadcastingUrl.replace("1-b",self.Live_Cluster_Backup)
                self.logi.appendMsg("INFO - ************** Going to stream BOTH PRIMARY AND BACKUP **************** ENTRY#" + str(i) + " , EntryId = " + str(self.entryId))
                # Stream to Primary
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd to primarySecuredBroadcastingUrl.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , primarySecuredBroadcastingUrl = " + str(Current_primarySecuredBroadcastingUrl))
                ffmpegCmdLine=self.liveObj.ffmpegCmdString(self.filePath, str(Current_primarySecuredBroadcastingUrl), self.streamUrl[i].streamName)
                rc,ffmpegOutputString1 = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user, self.remote_pass, ffmpegCmdLine, self.entryId,self.PublisherID,env=self.env,BroadcastingUrl=Current_primarySecuredBroadcastingUrl,FoundByProcessId=True)
                if rc == False:
                    self.logi.appendMsg("FAIL - Start_StreamEntryByShellScript - primarySecuredBroadcastingUrl. Start_StreamEntryByShellScript details: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , primarySecuredBroadcastingUrl = " + str(Current_primarySecuredBroadcastingUrl))
                    testStatus = False
                    return
                self.logi.appendMsg("PASS - Start_StreamEntryByShellScript - primarySecuredBroadcastingUrl is streaming OK. Details:  ENTRY#" + str(i) + " , entryId= " + self.entryId + " , primarySecuredBroadcastingUrl = " + str(Current_primarySecuredBroadcastingUrl))
                
                # Stream to Backup              
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd to secondarySecuredBroadcastingUrl.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + ", secondarySecuredBroadcastingUrl = " + str(Current_secondarySecuredBroadcastingUrl))
                ffmpegCmdLine=self.liveObj.ffmpegCmdString(self.filePath, str(Current_secondarySecuredBroadcastingUrl), self.streamUrl[i].streamName)
                rc,ffmpegOutputString = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user, self.remote_pass, ffmpegCmdLine, self.entryId,self.PublisherID,env=self.env,BroadcastingUrl=Current_secondarySecuredBroadcastingUrl,FoundByProcessId=True)
                if rc == False:
                    self.logi.appendMsg("FAIL - Start_StreamEntryByShellScript - secondarySecuredBroadcastingUrl. Start_StreamEntryByShellScript details: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + ", secondarySecuredBroadcastingUrl = " + str(Current_secondarySecuredBroadcastingUrl))
                    testStatus = False
                    return
                time.sleep(5)
                ffmpegOutputString = str(ffmpegOutputString1) + " " + (ffmpegOutputString)
                self.logi.appendMsg("PASS - Both PRIMARY + BACKUP are streaming OK -Start_StreamEntryByShellScript - secondarySecuredBroadcastingUrl.Details:  ENTRY#" + str(i) + " , entryId= " + self.entryId + " , secondarySecuredBroadcastingUrl = " + str(Current_secondarySecuredBroadcastingUrl))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '778','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '778','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_778_RTMPS_LiveNG_BothPrimaryAndBackup_Passthough.py -s')
    #===========================================================================
