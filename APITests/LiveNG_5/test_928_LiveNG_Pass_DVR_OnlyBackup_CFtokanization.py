...
@test_name: test_928_LiveNG_Pass_DVR_OnlyBackup_CFtokanization.py
 @desc : this test check E2E test of new LiveNG and CF tokanization requests permissions entries passthrough with DVR - including scroll to start point - REWIND ,Scroll to middle point - REWIND, BACK TO LIVE button
host access run ffmpeg cmd  - streaming only backup
...
DeliveryProfileIds:23723 ( Live Tokenized CF HLS(STG))

TESTING QA env-partner 6611:
applehttp VOD 1096 default aws hls,1113 hls short segment-simulive-aws
applehttp LIVE 1095 live ng,1129 live ng-CF Tokenized hls -->Access control =23016 QA env
mpegdash LIVE 1119 live ng-dash

PRODUCTION ENV - partner 2930571:
mpegdash	Live	16921 Live DASH
applehttp	Live	23723 Live Tokenized CF HLS(STG) -->Access control =25750412 PROD env
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.access_control_id_CFtokanization = self.inifile.RetIniVal('LiveNG_Partner_' + self.PublisherID,'access_control_id_CFtokanization')  # QA 23016#Prod 5750412
        ...
        self.logi = reporter2.Reporter2('test_928_LiveNG_Pass_DVR_OnlyBackup_CFtokanization')
        self.logi.initMsg('test_928_LiveNG_Pass_DVR_OnlyBackup_CFtokanization')
    ...
    def test_928_LiveNG_Pass_DVR_OnlyBackup_CFtokanization(self):
        global testStatus
        try:
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-LiveEntry_DVR_Trans_CFtokanization_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.passtrhroughId)
            ...
            time.sleep(5)
            # Set accessControl
            self.logi.appendMsg('INFO - Going to set AccessControl id  with CF tokanization' + str(self.access_control_id_CFtokanization) + 'EntryId =' + str(self.entry.id))
            base_entry = KalturaBaseEntry()
            base_entry.accessControlId = self.access_control_id_CFtokanization
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if rc == True:
                    ...
                    testStatus = False
            # Verify DVR
            #rc = self.liveObj.DVR_Verification(self.liveObj, self.entrieslst, self.entryId, self.MinToPlay,PlayerVersion=self.PlayerVersion)
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
            print ("fail")
            self.practitest.post(self.Practi_TestSet_ID, '928', '1', self.logi.msg)
            self.logi.reportTest('fail', self.sendto)
            assert False
        else:
            print ("pass")
            self.practitest.post(self.Practi_TestSet_ID, '928', '0', self.logi.msg)
    ...
    #===========================================================================
    #pytest.main(['test_928_LiveNG_Pass_DVR_OnlyBackup_CFtokanization.py','-s'])
    #===========================================================================
