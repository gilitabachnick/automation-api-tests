...
@test_name: test_917_LiveNG_SRT_VideoOnly_Passthrough.py
 
 @desc : This test check E2E test of new LiveNG entries SRT streaming both Primary +  Video only -> Same URL playback - Passthrough.
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_917_LiveNG_SRT_VideoOnly_Passthrough')
        self.logi.initMsg('test_917_LiveNG_SRT_VideoOnly_Passthrough')

    def test_917_LiveNG_SRT_VideoOnly_Passthrough(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-SRT_VideoOnly_PASS_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                ffmpegCmdLine,Current_primaryBroadcastingUrl=self.liveObj.ffmpegCmdString_SRT_VideoOnly(self.filePath, str(Current_primaryBroadcastingUrl), primarySrtStreamId)
            ...
            for CurrentplayerVersion in range(2,4):
                ...
                while datetime.datetime.now() <= limitTimeout and seenAll==False:   
                        if CurrentplayerVersion == 3:
                            time.sleep(5)
                            rc = self.liveObj_v3.verifyAllEntriesPlayOrNoBbyQrcode(entryList=self.entrieslst, boolShouldPlay=True,PlayerVersion=CurrentplayerVersion,ServerURL=self.ServerURL)
                        else:
                            time.sleep(2)
                            rc = self.liveObj.verifyAllEntriesPlayOrNoBbyQrcode(entryList=self.entrieslst, boolShouldPlay=True,PlayerVersion=CurrentplayerVersion,ServerURL=self.ServerURL)
                        time.sleep(5)
                        if not rc:
                            testStatus = False
                            return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '917','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '917','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_917_LiveNG_SRT_VideoOnly_Passthrough.py -s')
    #===========================================================================
