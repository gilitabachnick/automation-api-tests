...
@test_name: test_624_LiveNG_MultiAudio_Transcoding.py
 
 @desc : This test check E2E test of new LiveNG entries multi audio conversion by creating New Player v2 + v3
 ...
 Verify multi audio languageList="Spanish;English" selector on player v3/7
 Verify playback by Capture2Text
 Remark:For now player v2 is not supported
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 '''
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_624_LiveNG_MultiAudio_Transcoding')
        self.logi.initMsg('test_624_LiveNG_MultiAudio_Transcoding')
    ...
    def test_624_LiveNG_MultiAudio_Transcoding(self):
        global testStatus
        try: 
           # create client session
            ...
            # Create live object with current player
            self.liveObj_v3 = live.Livecls(None, self.logi, self.testTeardownclass, self.isProd, self.PublisherID, self.player_v3.id)

            self.entrieslst = []
            self.streamUrl = []
            # Create self.NumOfEntries live entries
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-AWS_MultiAudio_LiveEntry_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                #ffmpegCmdLine = self.liveObj.ffmpegCmdString_MultiAudio(self.filePath,str(Current_primaryBroadcastingUrl),self.entryId)
            ...
            for CurrentplayerVersion in range(3,4):#for now just player v3/7 is supported
                ...
                while datetime.datetime.now() <= limitTimeout and seenAll==False:
                        if CurrentplayerVersion == 3:
                            rc = self.liveObj_v3.verifyAllEntriesPlayOrNoBbyOnlyPlayback(self.entrieslst, True,PlayerVersion=CurrentplayerVersion,MultiAudio=True,ServerURL=self.ServerURL)
                        else:
                            rc = self.liveObj.verifyAllEntriesPlayOrNoBbyOnlyPlayback(self.entrieslst, True,PlayerVersion=CurrentplayerVersion,ServerURL=self.ServerURL)
                        time.sleep(5)
                        if not rc:
                            testStatus = False
                            return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '624','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '624','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_624_LiveNG_MultiAudio_Transcoding.py -s')
    #===========================================================================
