...
@test_name: test_918_LiveNG_SRT_AudioOnly_Passthrough.py
 
 @desc : This test check E2E test of new LiveNG entries SRT streaming both Primary +  audio only-> Same URL playback - Passthrough.
 ...
 SRT-Example for cmd line:
  ffmpeg -re -i /tmp/Disney_multi.mp4 -c copy -f mpegts "srt://srtlb.cluster-1-a.live.nvq1.ovp.kaltura.com:7045?streamid=#:::e=0_u4wd748d,st=0,p=b3bdad08"
 srtlb.cluster-1-a.live.nvq1.ovp.kaltura.com:7045
 srtlb.cluster-1-b.live.nvq1.ovp.kaltura.com:7045
    e - entry id
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_918_LiveNG_SRT_AudioOnly_Passthrough')
        self.logi.initMsg('test_918_LiveNG_SRT_AudioOnly_Passthrough')

    def test_918_LiveNG_SRT_AudioOnly_Passthrough(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-SRT_AudioOnly_PASS_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                # Stream to Primary
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd to primaryBroadcastingUrl.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , primaryBroadcastingUrl = " + str(self.streamUrl[i].primaryBroadcastingUrl))
                ffmpegCmdLine,Current_primaryBroadcastingUrl=self.liveObj.ffmpegCmdString_SRT_AudioOnly(self.filePath, str(Current_primaryBroadcastingUrl), primarySrtStreamId)
                ...
                if rc == False:
                    self.logi.appendMsg("FAIL - Start_StreamEntryByShellScript - primaryBroadcastingUrl. Start_StreamEntryByShellScript details: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , primaryBroadcastingUrl = " + str(Current_primaryBroadcastingUrl))
                    testStatus = False
                    return
                self.logi.appendMsg("PASS - Start_StreamEntryByShellScript - primaryBroadcastingUrl is streaming OK. Details:  ENTRY#" + str(i) + " , entryId= " + self.entryId + " , primaryBroadcastingUrl = " + str(Current_primaryBroadcastingUrl))
                ...
                rc = self.LiveDashboard.Verify_LiveDashboard(self.logi, self.LiveDashboardURL, self.entryId,self.PublisherID,self.ServerURL,self.UserSecret,self.env, Flag_Transcoding=False,Live_Cluster_Primary=self.Live_Cluster_Primary,ServersStreaming="Only_Primary")
                if rc == True:
            ...
            for CurrentplayerVersion in range(2, 4):
                ...
                self.logi.appendMsg("INFO - ************** Going to play live entries with with SRT AudioOnly=True on PlayerVersion= " + str(CurrentplayerVersion) + "  on preview&embed page during - MinToPlay=" + str(self.MinToPlay) + " , Start time = " + str(datetime.datetime.now()))
            ...
            self.logi.appendMsg("PASS - Playback with SRT AudioOnly=True of PlayerVersion= " + str(CurrentplayerVersion) + " live entries on preview&embed page during - MinToPlay=" + str(self.MinToPlay) + " , End time = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '918','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '918','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_918_LiveNG_SRT_AudioOnly_Passthrough.py -s')
    #===========================================================================
