...
@test_name: test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault.py
  @desc : this test check E2E test of  test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault - Create simulive event by kaltura API and then play the entry on LIVE logic platform
 ...
 MainEntryId.mp4     --> duration 5
 postEndEntryId.mp4  --> duration 10
 ...
 * You can find files on /home/kaltura/entries/Simulive_To_Live_Files
 ...
 Verify that there is NO playback of live streaming on the MAIN area of the simulive entry.
 3)Default case (isContentInterruptibleFalse)-->Verify that just after arriving to POST area -> The live entry content is played on the simulive entry.
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.MainEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleFalseDefault_MainEntryId_Daul5min')  # QA "0_m5aia7c7" #PROD "1_n6mie47o"#5 min duration - File AUTOMATIONmainForDual_5min.mp4
        self.preStartEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleFalseDefault_preStartEntryId_Ron1min')  # QA "0_nwuqpww7" #PROD "1_b2x73t0y" # 1 minutes duration - File LongCloDvRec_Pre_1min.mp4
        self.postEndEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleFalseDefault_postEndEntryId_Dual10min')  # QA "0_m8f9f3cy"  #PROD"1_mbm0aeve" #10 min duration - File AUTOMATIONmainForDual_10min.mp4
        ...
        self.sleepUntilCloseBrowser = 600  # 10min*60 #(seconds) for waiting to close the browser after playback verification
        ...
        self.logi = reporter2.Reporter2('test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault')
        self.logi.initMsg('test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault')
    
    def test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault(self):
        global testStatus
        try: 
           # create client session
            ...
            if self.env == 'prod':
                time.sleep(20)
            PlayBrowser = None
            self.logi.appendMsg("INFO - *************** Verify NO playback of SIMULIVE TO LIVE Playback on MAIN entry area(after start streaming - boolShouldPlay=False): " + str(self.entryId) + " , TimeNow = " + str(datetime.datetime.now()))
            rc_SimuliveToLive = self.liveObj.verifyLiveisPlayingOverTime_Sniffer(PlayBrowser=PlayBrowser,entryList=self.entrieslst,boolShouldPlay=False,MinToPlayEntry=1,sniffer_fitler=self.sniffer_fitler_SimuliveVOD,QRCode=True,verifyPlayback=True)
            if rc_SimuliveToLive == True:
                self.logi.appendMsg("PASS  - verifyLiveisPlayingOverTime_Sniffer")
            else:
                self.logi.appendMsg("FAIL - verifyLiveisPlayingOverTime_Sniffer:boolShouldPlay=False")
                testStatus = False
            ########################
            self.logi.appendMsg("INFO  - Wait for arriving to POST ENTRY time area.")
            while (startTime_SimulivePlayback+((self.preStartEntryId_duration + self.SchedulerEvent_MainEntryId_duration)*60)) > time.time():
               time.sleep(5)
            time.sleep(30)  # (buffering time)Wait for the stream to start playing on the simulive entry after arriving to post area
            self.logi.appendMsg("INFO - *************** Verify playback of SIMULIVE TO LIVE Playback on POST entry area(after start streaming - klive): " + str(self.entryId) + " , TimeNow = " + str(datetime.datetime.now()))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '914','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '914','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_914_SimuliveToLiveMAIN_isContentInterruptibleFalseDefault.py -s')
    #===========================================================================
