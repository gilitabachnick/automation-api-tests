...
@test_name: test_851_LiveNG_DuplicationPreventionWithoutChannelid_MultiFlavors.py
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.sniffer_filter_per_flavor_list = '-s32;-s32;-s36;-s37'# ts search for each flavor on flavorList - Delivery profile 1033:Live packager hls -redirect
        self.MAP_filePath = "/home/kaltura/entries/multiflavors/"
        ...
        self.logi = reporter2.Reporter2('test_851_LiveNG_DuplicationPreventionWithoutChannelid_MultiFlavors')
        self.logi.initMsg('test_851_LiveNG_DuplicationPreventionWithoutChannelid_MultiFlavors')
    ...
    def test_851_LiveNG_DuplicationPreventionWithoutChannelid_MultiFlavors(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-DuplicationPrevention_MultiFLAVORS-WithoutChannelId' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd - Only primaryBroadcastingUrl.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.MAP_filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , primaryBroadcastingUrl = " + str(Current_primaryBroadcastingUrl))
                ffmpegCmdLine = self.liveObj.ffmpegCmdString_MultiFlavors_MAP_WithoutChannelid(str(Current_primaryBroadcastingUrl))
                ffmpegCmdLine = "cd " + self.MAP_filePath + ";" + ffmpegCmdLine  # Go to map files path for running ffmpeg cmd line
                #start ffmpeg by FoundByProcessId
                rc1, ffmpegOutputString1 = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user,self.remote_pass, ffmpegCmdLine,self.entryId, self.PublisherID,env=self.env, BroadcastingUrl=str(Current_primaryBroadcastingUrl), FoundByProcessId=True,MultiArrProcessIds=True)
                if rc1 == False:
                    self.logi.appendMsg("FAIL - Start_StreamEntryByShellScript. Start_StreamEntryByShellScript details: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.MAP_filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId))
                    testStatus = False
                    return
                time.sleep(5)
                rc2, ffmpegOutputString2 = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user,self.remote_pass, ffmpegCmdLine,self.entryId, self.PublisherID,env=self.env, BroadcastingUrl=str(Current_primaryBroadcastingUrl), FoundByProcessId=True,timout_SearchPsAux=60,MultiArrProcessIds=True)
            ...
            self.logi.appendMsg("INFO - Going to end streams by ByProcessId cmd.Datetime = " + str(datetime.datetime.now()) + " , FoundByProcessId = " + str(ffmpegOutputString1[0]))
            ...
            if rc != False:            
                self.logi.appendMsg("PASS - streamEntryByShellScript and VerifyResultFromStreamEntryByShellScript.  streamEntryByShellScript: remote_host= " + self.remote_host + " , remote_user= " + self.remote_user + " , remote_pass= " + self.remote_pass + " , MAP_filePath= " + self.MAP_filePath + " , entryId= " + self.entryId + " , entryId= " + self.PublisherID + " , playerId= " + str(self.playerId))
            else:
                self.logi.appendMsg("FAIL - streamEntryByShellScript and VerifyResultFromStreamEntryByShellScript. streamEntryByShellScript: remote_host= " + self.remote_host + " , remote_user= " + self.remote_user + " , remote_pass= " + self.remote_pass + " , MAP_filePath= " + self.MAP_filePath + " ,entryId= " + self.entryId + " , entryId= " + self.PublisherID + " , playerId = " + str(self.playerId))
                testStatus = False
                return    
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '851','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '851','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_851_LiveNG_DuplicationPreventionWithoutChannelid_MultiFlavors.py -s')
    #===========================================================================
