...
@test_name: test_468_LiveNG_MultiFlavors_MAPSwitchFlavors_RestartUnder3min.py
...
class TestClass:
    ...
    def setup_class(self):
        ...
        #self.sniffer_filter_per_flavor_list = '-s32.ts;-s32.ts;-s36.ts;-s37.ts'  # ts search for each flavor on flavorList - Delivery profile 1096 LIVE NG
        ...
        self.logi = reporter2.Reporter2('test_468_LiveNG_MultiFlavors_MAPSwitchFlavors_RestartUnder3min')
        self.logi.initMsg('test_468_LiveNG_MultiFlavors_MAPSwitchFlavors_RestartUnder3min')
    ...
    def test_468_LiveNG_MultiFlavors_MAPSwitchFlavors_RestartUnder3min(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-MultiFlavors_MAPSwitchFlavors_RestartUnder3Min' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                ffmpegCmdLine=self.liveObj.ffmpegCmdString_MultiFlavors_MAP(self.PublisherID,  str(Current_primaryBroadcastingUrl), self.streamUrl[i].streamName)
            ...
            # kill ffmpeg ps   
            self.logi.appendMsg("INFO - Going to end streams by kill FoundByProcessId cmd.Datetime = " + str(datetime.datetime.now()))
            ...
            # Restart steaming
            self.logi.appendMsg("INFO - ************** Going to RESTART STREAMING after " + str(self.RestartDowntime) + "  - Start_StreamEntryByffmpegCmd.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.source_file1 + " ," + self.source_file2 + " ," + self.source_file3 + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '468','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '468','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_468_LiveNG_MultiFlavors_MAPSwitchFlavors_RestartUnder3min.py -s')
    #===========================================================================
