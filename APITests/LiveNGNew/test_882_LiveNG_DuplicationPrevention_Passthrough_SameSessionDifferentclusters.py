...
@test_name: test_882_LiveNG_DuplicationPrevention_Passthrough_SameSessionDifferentclusters.py
...
class TestClass:
    ...
    def setup_class(self):
        ...
        if self.PartnerID != None:
            ...
            self.PublisherID = self.inifile.RetIniVal(self.section, 'LIVENG_hybridcdn_PublisherID')  # QA 9025105/ PROD
           ...
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if self.env == 'testing':
                    if self.ConfigId=="":
                        if self.Live_Change_Cluster == True:
                            primaryBroadcastingUrl_DifferentClusters = primaryBroadcastingUrl_DifferentClusters.replace(entryId_Prefix + ".p.rtmp.publish","rtmp-0.cluster-" + self.Live_Cluster_Backup + ".p.rtmp.publish")
                        else:  # default case
                            primaryBroadcastingUrl_DifferentClusters = primaryBroadcastingUrl_DifferentClusters.replace(entryId_Prefix + ".p.rtmp.publish","rtmp-0.cluster-1-b")
                        primaryBroadcastingUrl_DifferentClusters = primaryBroadcastingUrl_DifferentClusters + "&i=0&e=" + self.entryId
                    else:#Multi region
                        if self.Live_Change_Cluster == True:
                            primaryBroadcastingUrl_DifferentClusters = primaryBroadcastingUrl_DifferentClusters.replace(self.ConfigId + "-" + entryId_Prefix + ".p.rtmp.publish","rtmp-0.cluster-" + self.Live_Cluster_Backup + ".p.rtmp.publish")
                        else:  # default case
                            primaryBroadcastingUrl_DifferentClusters = primaryBroadcastingUrl_DifferentClusters.replace(self.ConfigId + "-" + entryId_Prefix + ".p.rtmp.publish","rtmp-0.cluster-1-b")
                        primaryBroadcastingUrl_DifferentClusters=primaryBroadcastingUrl_DifferentClusters.replace(self.entryId,self.ConfigId + "-" + self.entryId) + "&i=0&e=" + self.entryId
                ...
                if self.IsSRT == True:
                    ...
                    ffmpegCmdLine = self.liveObj.ffmpegCmdString(self.filePath,str(primaryBroadcastingUrl_DifferentClusters),self.streamUrl[i].streamName)
                rc2, ffmpegOutputString2 = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user,self.remote_pass, ffmpegCmdLine,self.entryId, self.PublisherID,env=self.env,BroadcastingUrl=primaryBroadcastingUrl_DifferentClusters,FoundByProcessId=True,timout_SearchPsAux=60,MultiArrProcessIds=True)
                if ffmpegOutputString2.find("NoProcessId_")>=0:
                    ...
                    if len(ffmpegOutputString2)>1:
                        ...
                        return
                if rc2 == False:
                    self.logi.appendMsg("INFO - Second stream was disconnected Start_StreamEntryByShellScript. Start_StreamEntryByShellScript details: " + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId))
                    #testStatus = False
                    #return
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '882','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '882','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True


    #===========================================================================
    #pytest.main('test_882_LiveNG_DuplicationPrevention_Passthrough_SameSessionDifferentclusters.py -s')
    #===========================================================================
