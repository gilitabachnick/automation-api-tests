...
@test_name: test_812_LiveNG_Transcoding_DVR_V7Player.py
 @desc : this test check E2E test of new LiveNG entries Transcoding with DVR - including scroll to start point - REWIND ,Scroll to middle point - REWIND, BACK TO LIVE button
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_812_LiveNG_Transcoding_DVR_V7Player')
        self.logi.initMsg('test_812_LiveNG_Transcoding_DVR_V7Player')
    ...
    def test_812_LiveNG_Transcoding_DVR_V7Player(self):
        global testStatus
        try:
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-LiveEntry_DVR_PASSTHROUGH_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                if rc == True:
                    ...
                    testStatus = False
            # Verify DVR
            rc= self.liveObj.DVR_Verification(liveObj=self.liveObj, entryList=self.entrieslst, entryId=self.entryId, MinToPlay=self.MinToPlay, PlayerVersion=self.PlayerVersion,ServerURL=self.ServerURL)
            if rc == True:
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
            print ("fail")
            self.practitest.post(self.Practi_TestSet_ID, '812', '1', self.logi.msg)
            self.logi.reportTest('fail', self.sendto)
            assert False
        else:
            print ("pass")
            self.practitest.post(self.Practi_TestSet_ID, '812', '0', self.logi.msg)
    ...
    #===========================================================================
    #pytest.main(['test_812_LiveNG_Transcoding_DVR_V7Player.py','-s'])
    #===========================================================================
