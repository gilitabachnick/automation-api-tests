...
@test_name: test_431_LiveNG_BothPrimaryAndBackup_Passthough.py.py
 
 @desc : This test check E2E test of new LiveNG entries streaming both Primary +  Backup -> Same URL playback - Passthough. 
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_431_LiveNG_BothPrimaryAndBackup_Passthough')
        self.logi.initMsg('test_431_LiveNG_BothPrimaryAndBackup_Passthough')
    ...
    def test_431_LiveNG_BothPrimaryAndBackup_Passthough(self):
        global testStatus
        try: 
           # create client session
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-LiveEntry_BOTHPRIMARYBACKUP_PASS_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "adi category", 1,None,self.passtrhroughId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                rc,ffmpegOutputString1 = self.liveObj.Start_StreamEntryByffmpegCmd(self.remote_host, self.remote_user, self.remote_pass, ffmpegCmdLine, self.entryId,self.PublisherID,env=self.env,BroadcastingUrl=Current_primaryBroadcastingUrl,FoundByProcessId=True)
                if rc == False:
                ...
                # Stream to Backup              
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd to secondaryBroadcastingUrl.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + ", secondaryBroadcastingUrl = " + str(Current_secondaryBroadcastingUrl))
                if self.IsSRT == True:
                ...
                self.logi.appendMsg("PASS - Both PRIMARY + BACKUP are streaming OK -Start_StreamEntryByShellScript - secondaryBroadcastingUrl.Details:  ENTRY#" + str(i) + " , entryId= " + self.entryId + " , primaryBroadcastingUrl = " + str(Current_secondaryBroadcastingUrl))
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '431','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '431','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_431_LiveNG_BothPrimaryAndBackup_Passthough.py -s')
    #===========================================================================
