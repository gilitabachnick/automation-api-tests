...
@test_name: test_915_SimuliveToLiveMAIN_isContentInterruptibleTrue.py
  @desc : this test check E2E test of  test_915_SimuliveToLiveMAIN_isContentInterruptibleTrue - Create simulive event by Kaltura API and then play the entry on LIVE logic platform
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.MainEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleTrue_MainEntryId_Daul10min')  # QA "0_m8f9f3cy" #PROD "1_mbm0aeve"#10 min duration - File AUTOMATIONmainForDual_10min.mp4
        self.preStartEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleTrue_preStartEntryId_Ron1min')  # QA "0_nwuqpww7" #PROD "1_b2x73t0y" # 1 minutes duration - File LongCloDvRec_Pre_1min.mp4
        self.postEndEntryId = self.inifile.RetIniVal('Simulive_Partner_' + self.PublisherID,'SimuliveToLiveMAIN_isContentInterruptibleTrue_postEndEntryId_Dual5min')  # QA "0_m5aia7c7"  #PROD"1_n6mie47o" #5 min duration - File AUTOMATIONmainForDual_5min.mp4
        ...
        self.isContentInterruptible = True #Verify that after arriving to MAIN area(isContentInterruptible = True) -> The live entry content is played on the simulive entry.
        ...
        try: 
           # create client session
            ...
            rc, LiveEntryId = self.SimuliveObj.Simulive_Add(client=self.client, UserSecret=self.UserSecret,
                                                        ...
                                                        postEndEntryId=self.postEndEntryId,isContentInterruptible=self.isContentInterruptible)
            if rc == True:
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '915','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '915','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_915_SimuliveToLiveMAIN_isContentInterruptibleTrue.py -s')
    #===========================================================================
