...
@test_name: test_486_LiveNG_E2ESourceOnly.py
 
 @desc : This test check E2E test of new LiveNG entries source only by creating New Player v2 + v3 
...
class TestClass:
    ...
    def setup_class(self):
        ...
        self.logi = reporter2.Reporter2('test_486_LiveNG_E2ESourceOnly')
        self.logi.initMsg('test_486_LiveNG_E2ESourceOnly')
    ...
    def test_486_LiveNG_E2ESourceOnly(self):
        global testStatus
        try:
            ...
            self.logi.appendMsg('INFO - Going to create(if not exist) conversion profile = Live source only')
            Transobj = Transcoding.Transcoding(self.client, 'Live source only')
            self.CloudtranscodeId=Transobj.CreateConversionProfileFlavors(self.client,'Live source only','32')
            ...
            for i in range(0, self.NumOfEntries):
                Entryobj = Entry.Entry(self.client, 'AUTOMATION-AWS_LINUX_LiveEntry_' + str(i) + '_' + str(datetime.datetime.now()), "Live Automation test " + str(self.NumOfEntries) + " Entries", "Live tag", "Admintag", "Moran category", 1,None,conversionProfileId=self.CloudtranscodeId)
            ...
            for i in range(0, self.NumOfEntries):
                ...
                self.logi.appendMsg("INFO - ************** Going to ssh Start_StreamEntryByffmpegCmd.********** ENTRY#" + str(i) + " , Datetime = " + str(datetime.datetime.now()) + " , "  + self.remote_host + " , " + self.remote_user + " , " + self.remote_pass + " , " + self.filePath + " ," + self.entryId + " , " + self.PublisherID + " , " + str(self.playerId) + " , BroadcastingUrl= " + str(Current_primaryBroadcastingUrl))
                if self.IsSRT == True:
            ...
            if rc != False:            
                self.logi.appendMsg("PASS - End_StreamEntryByffmpegCmd.streamEntryByShellScript: remote_host= " + self.remote_host + " , remote_user= " + self.remote_user + " , remote_pass= " + self.remote_pass + " , filePath= " + self.filePath + " , entryId= " + self.entryId + " , entryId= " + self.PublisherID + " , playerId= " + str(self.playerId))
            else:
                self.logi.appendMsg("FAIL - End_StreamEntryByffmpegCmd.streamEntryByShellScript: remote_host= " + self.remote_host + " , remote_user= " + self.remote_user + " , remote_pass= " + self.remote_pass + " , filePath= " + self.filePath + " ,entryId= " + self.entryId + " , entryId= " + self.PublisherID + " , playerId = " + str(self.playerId))
                testStatus = False
                return    
    ...
    def teardown_class(self):
        ...
        if testStatus == False:
           print("fail")
           self.practitest.post(self.Practi_TestSet_ID, '486','1', self.logi.msg)
           self.logi.reportTest('fail',self.sendto)        
           assert False
        else:
           print("pass")
           self.practitest.post(self.Practi_TestSet_ID, '486','0', self.logi.msg)
           self.logi.reportTest('pass',self.sendto)
           assert True        
    
            
    #===========================================================================
    #pytest.main('test_486_LiveNG_E2ESourceOnly.py -s')
    #===========================================================================
